...
use syn::{
    parse_str, Expr, ExprLit, Ident, Index, Lit, LitByteStr, LitStr, Meta, MetaNameValue, Path,
};
...
impl StringTy {
    ...
    fn rust_type(&self) -> TokenStream {
        match self {
            Self::String => quote! { ::prost::alloc::string::String },
            Self::ByteString => quote! { ::prost::bytestring::ByteString },
        }
    }
}
        ...
        let default = match lit {
            ...
            Lit::Str(ref lit)
                if *ty == Ty::String(StringTy::String)
                    || *ty == Ty::String(StringTy::ByteString) =>
            {
...
impl ToTokens for DefaultValue {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        match *self {
            ...
            DefaultValue::String(ref value) => {
                let s = LitStr::new(value, Span::call_site());
                tokens.append_all(quote!(#s as &str));
            }
            DefaultValue::Bytes(ref value) => {
